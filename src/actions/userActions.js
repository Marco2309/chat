...
export const getConvertations = (MyUser) => async (dispatch) => {
  ...
  if (MyUser !== undefined && MyUser._id !== undefined) {
    const myConversation = allConversationsData.filter((conversation) =>
      conversation.members.includes(MyUser._id)
    );
    ...
    const idUsersMyConversations = myConversation.map((conversation) =>
      conversation.members.find((id) => id !== MyUser._id)
    );
